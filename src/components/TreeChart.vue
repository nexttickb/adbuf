<template>
    <table v-if="treeData.name">
      <tr>
        <td @dblclick="nodeClick(treeData)" :colspan="Array.isArray(treeData.children) ? treeData.children.length * 2 : 1" 
		...
		getTreeList(nodeData){
			...
			RUNKIT.ServerApi.send(this.api.treeData, {where:nodeData || {}}, (bl, res)=>{
				...
				for(let i = 0; i < res.list.length; i++){
					...
					node['name'] = node[this.treeConf.label];
...
.node .person{position: relative; display: inline-block;z-index: 2;width:2em; overflow: hidden;}
.node .person .avat{display: block;width:2em;height:2em;margin:auto;overflow:hidden; background:#999;border:1px solid #ccc;box-sizing: border-box;}

.node .person .name{height:1em;line-height: 1em;font-size:0.6rem;overflow: hidden;width:100%;}
