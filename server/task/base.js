...
class spider{
	...
	constructor(){
		this.typeKv = {};
		this.maxId = types.length;
		for(let i  = 0; i < types.length; i++){
			this.typeKv[types[i].type_name] = types[i].type_id;
		}
	}
	log(){
	...
	async getDetail(id, title, url){
		...
		movie.vod_play_url = [];
		$($('.vodplayinfo ul')[0]).find('li').each(async (index, ele)=>{
			let text = $(ele).text();
			log(index, text);
			let kv = text.split('$');
	...
	async getList(page, db){
		let $ = await get$('http://www.mahuazy.com/?m=vod-index-pg-' + page + '.html')
		if(!$)return false;
		$('.xing_vb4 a').each(async (index, ele)=>{
			let url = $(ele).attr('href');
			let name = $(ele).html();
			let id = url.substring(18).substr(0, url.substring(18).length - 5);
			let movie = await getDetail(id, name,'http://www.mahuazy.com' + url);
			...
			console.log('insert:', index, page, movie.vod_name, movie.type_name, movie.type_id);
